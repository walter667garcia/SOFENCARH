...
namespace Capa_Vista.Localizacion
{
    public partial class frmUbicacion : Form
    {
        ...
        private static UbicacionFormulario instanciaAbierta = null;
        ...
        private void OcultarColumnas()
        {
            try
            {
                ...
                if (this.dtgUbicacion.Columns.Count > 0)
                {
                    this.dtgUbicacion.Columns[0].Visible = false;
                }
                else
                {
        ...
        private void dtgUbicacion_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            {
                var hti = dtgUbicacion.HitTest(e.X, e.Y);
                if (hti.RowIndex >= 0)
                {
                    dtgUbicacion.Rows[hti.RowIndex].Selected = true;
                }
            }
                ...
                else if (id.Contains("Eliminar"))
                {
                    if (instanciaAbierta == null || instanciaAbierta.IsDisposed)
                    {
                        ...
                        if (result == DialogResult.Yes)
                        {
                            ...
                            string rpta = "";
                            int Id = Convert.ToInt32(this.dtgUbicacion.CurrentRow.Cells["ID"].Value);
                            rpta = nLocalizacion.Eliminar(Id);
                            if (rpta.Equals("OK"))
                            {
